			...
			if (projectFile) {
				...
				if (dataURIs["project.json"]) {
					...
					for (var target of data.targets) {
						for (var costume of target.costumes) { //check the costumes for their asset filenames.
							assets[costume.md5ext] = "/assets/" + costume.md5ext + "?n=1"; //use ?n=1 at end for automatic cache clearing for github
						}
						for (var sound of target.sounds) { //check the sounds for their asset filenames.
